...
const Modal = ({ onClose }: any) => (
  <div className="flex flex-col w-[40px] border">
    <button onClick={onClose} className="w-auto p-1 border-b text-[12px]">
      수정
    </button>
    <button onClick={onClose} className="w-auto p-1 text-[12px]">
      삭제
    </button>
  </div>
);
  ...
  return (
    <div className="flex">
      <div className="float-left">
        {follow === false ? (
          <button type="submit" className={btnfollowcss} onClick={followClick}>
            팔로우
          </button>
        ) : (
          <button type="submit" className={btnfollowcancelcss} onClick={followCancelClick}>
            팔로잉
          </button>
        )}
      </div>
