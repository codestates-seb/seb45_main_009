...
const MyPageFeed = () => {
  ...
  const IntroductionCss =
    " border border-bdc rounded-xl  p-10 h-full min-h-[200px] m-8 w-[50%] md:w-auto ";
      ...
      >
        {userInfo.userType === "USER" ? (
          <aside className="flex flex-col px-6 py-10 mr-4 max-768:flex-row">
            ...
            <div className="flex flex-col max-768:justify-center">
              <div className="flex justify-center font-semibold text-lg mb-4 max-mobile:text-sm">
                {user?.nickname}
              </div>
              ...
              <div className="mb-2 max-mobile:text-sm max-mobile:mb-0">
                <span className="max-w-[200]">
                  <span className="font-bold mr-2">주 종목</span>{" "}
                  <span>{user?.sport ? user?.sport : "없음"}</span>
                </span>
              </div>
          ...
          <aside className="flex flex-col px-4 mr-4 max-768:flex-row">
            ...
            <div className="flex flex-col max-768:justify-center max-w-[200px]">
              ...
              <div className="mb-2 max-mobile:text-sm max-mobile:mb-0">
                <span className="max-w-[200]">
                  <span className="font-bold mr-2">주소</span>{" "}
                  <span>{user?.location ? user?.location : "없음"}</span>
                </span>
              </div>
              <div className="mb-2 max-mobile:text-sm max-mobile:mb-0">
                <span className="max-w-[200]">
              ...
              <div className="mb-2 max-mobile:text-sm">
                <span className="max-w-[200]">
                  <span className="font-bold mr-2">소개</span>{" "}
                  <span>{user?.bio ? user?.bio : "없음"}</span>
                </span>
              </div>
              <div className="mb-2 max-mobile:text-sm">
                <span className="max-w-[200]">
                  <span className="font-bold mr-2">가격 정보</span>{" "}
                  <span>{user?.price ? user?.price : "없음"}</span>
                </span>
              </div>
            </div>
          </aside>
        ...
        {!loading ? (
          userFeed.length !== 0 ? (
            ...
            <div>
              <img src={noFeed} alt="noFeedImage" />
            </div>
          )
        ) : (
