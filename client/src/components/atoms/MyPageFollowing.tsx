...
const MyPageFollowing: React.FC<MyPageFollowerProps> = ({ inView }) => {
  ...
  const getUserData = async () => {
    ...
    try {
      ...
      const response = await globalAxios.get(
        `/follow/following/${userInfo.userId}`,
    ...
    <div className="flex justify-center flex-col items-center mt-5">
      <section className="grid  grid-cols-1 gap-8 mb-24 mx-3 sm:grid-cols-2 sm:gap-6 md:gap-8 lg:grid-cols-3 2xl:grid-cols-4 w-[80%]">
        <div className="flex items-center">
          ...
          <p className="text-2xl font-bold ml-4">팔로잉 목록</p>
        </div>
        <span className="col-span-1 lg:col-span-2 2xl:col-span-3"></span>
        <span className="col-span-1 lg:col-span-2 2xl:col-span-3"></span>
        <div className="flex justify-end w-full">
          <Link to={"/mypage/follow/follower"}>
            <div className={`mr-10 cursor-pointer`}>팔로워</div>
          </Link>
          <div className={`mr-10 cursor-pointer ${"text-blue-500"}`}>팔로잉</div>
        </div>
...
export default MyPageFollowing;
